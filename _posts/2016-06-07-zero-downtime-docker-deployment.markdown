...
nginx는 무료지만 훌륭한 성능을 자랑하는 로드밸런서기도 합니다. `80포트`로 들어온 요청을 `8080포트`, `8081포트`로 분산할 수 있고 health check를 통해 포트가 죽어있다면 살아있는 포트로 요청을 보내게 됩니다.
...
blue container가 실행되면 도커가 이벤트를 docker-gen에 전달하게 되고 docker-gen은 blue container 정보를 추가하여 nginx 설정 파일을 만들고 nginx를 재기동합니다. 그 후 green container가 중지되면 역시 도커가 이벤트를 docker-gen에 전달하게 되고 docker-gen은 green container 정보를 제거한 nginx 설정 파일을 만들고 nginx를 재기동합니다.
...
어쩌다보니 도커에 대한 내용이 더 많아진 느낌입니다. 이미 도커를 알고 계신분은 심심한 포스트일지도.. 하지만, 도커를 몰랐던 분들은 꼭 써보세요. 좋습니다! 도커에 대한 내용은 [여기](http://documents.docker.co.kr/)에서 더 많이 볼 수 있습니다.
